  ...
  -  锁膨胀/锁优化分析之一： 索引等值匹配  加锁规则
      - 锁膨胀/锁优化分析 分为两种情况：
       ...
       不同索引类型下 索引等值与 索引不等值的区别
        ...
        - **普通索引**:
          索引等值,对所有匹配的 索引记录 都上了 Next-key Lock，查询到的 聚族索引记录 都上了Record Lock， 另外，对匹配索引记录的next key记录上了Gap Lock。（相当于把自身和前后的间隙都加锁了）原因是普通索引可能重复，需要向右扫描
          索引不等值: 行级锁 会上一把 间隙锁，与聚集索引和唯一索引的规则相同

      范围查询加锁规则
       - 在 InnoDB 中，范围查询的加锁行为取决于是否命中索引、索引类型及隔离级别（默认RR），具体对应关系如下表所示：

| 查询条件       | 索引类型       | 索引记录锁类型       | 聚集索引锁类型       | 说明                                                                 |
|----------------|----------------|----------------------|----------------------|----------------------------------------------------------------------|
| >= (等值部分)   | 聚集索引       | Record Lock          | Record Lock          | 等值匹配触发Next-Key Lock降级优化，仅锁定当前记录索引                 |
| 其他范围操作   | 聚集索引       | Next-key Lock        | Record Lock          | 非等值匹配使用临键锁（记录锁+间隙锁），防止幻读                       |
| 范围操作       | 唯一索引       | Next-key Lock        | Record Lock          | 唯一索引无重复值，但范围查询仍需间隙锁防止插入新记录                 |
| 范围操作       | 普通索引       | Next-key Lock        | Record Lock          | 普通索引可能重复，需锁定匹配记录及间隙，避免幻读                     |
| 非聚簇索引查询 | 未使用索引     | 全表 Next-key Lock   | 全表 Record Lock（表锁） | 未使用索引时行锁退化为表锁（锁膨胀），严重影响并发性能               |

         对于聚簇索引，只有 >= 的等值部分会触发锁降级；
         而对于二级索引（唯一索引/普通索引），所有匹配记录及对应聚集索引记录都会被加锁，并且默认使用 Next-Key Lock 来防止幻读。
         若未使用聚簇索引（如全表扫描），则可能导致“行锁变表锁”的严重后果，影响系统并发性能（锁膨胀）。